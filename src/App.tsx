...
export default function App() {
  ...
  const generateAIResponse = (userInput: string) => {
    ...
    // 2. Confirm Full Report
    else if (stage === 2 && (userInput.includes('需要') || userInput.includes('是') || userInput.includes('生成') || userInput.includes('好'))) {
      const msg1: Message = {
        id: id(),
        role: 'assistant',
    ...
    // 3. Confirm Recommendation (District)
    else if (stage === 4 && (userInput.includes('需要') || userInput.includes('是') || userInput.includes('推荐') || userInput.includes('好'))) {
      const msg1: Message = {
        id: id(),
        role: 'assistant',
    ...
    // 4. Confirm Business Opportunity
    else if (stage === 5 && (userInput.includes('生成') || userInput.includes('是') || userInput.includes('好') || userInput.includes('要'))) {
      const msg1: Message = {
        id: id(),
        role: 'assistant',
        content: "已为您在系统中成功生成【南京新街口商圈】展业商机！您可以随时在工作台中追踪该商圈的拓展进度。\n\n接下来，进入**商机推进策略阶段**。是否为您推荐适销产品并进行竞品对标分析？",
      };
      setMessages(prev => [...prev, msg1]);
      setStage(6); 
    }
    // 5. Strategy: Product Recommendation + Comparison Table
    else if (stage === 6 && (userInput.includes('推荐') || userInput.includes('是') || userInput.includes('好') || userInput.includes('要'))) {
      const msg1: Message = {
        id: id(),
        role: 'assistant',
        content: `针对南京新街口商圈的高端餐饮及连锁业态，帮帮为您定制了**【商圈餐饮安心统保方案】**。

### 核心推荐：餐饮行业一站式综合保障方案
**方案亮点：**
- **全维度覆盖**：整合“财产+雇主+公责+食责+营断”，一份保单解决餐饮经营全链路风险。
- **数字化极效**：支持线上秒级核保，适配商圈快节奏展业，1步完成投保闭环。
- **风险减量服务**：深度嵌入燃气检测、消防风勘及食安培训，变“事后补偿”为“事前预防”。

**为什么适合新街口商圈？**
- **适配连锁扩张**：新街口连锁化率超80%，方案支持“3家以上分店保费直降20%”，极具价格竞争力。
- **对标高标合规**：德基、金鹰等顶级商业体对商户合规要求严苛，本方案的增值服务能有效协助商户通过商场安检。
- **对冲高流量风险**：针对“中华第一商圈”的超高人流量，强化了公责与烫伤附加险，精准覆盖核心痛点。

---

### 🛡️ 平安 VS 人保：核心竞品深度对标分析

| 对比维度 | **平安生意兴隆保 (基础款)** | 人保餐饮业经营者责任保险 | **平安核心优势** |
| :--- | :--- | :--- | :--- |
| **保障范围** | 财产 + 员工意外 + 第三者责任，可附加食品饮料责任 | 仅覆盖第三者责任（食物中毒/人身伤亡），无财产/员工保障 | **一站式全覆盖**，无需单独投保多份产品，贴合餐饮全风险需求 |
| **核保效率** | 线上秒核，仅需营业执照，自主选生效日期 | 线下人工核保，需提交全套门店资质，耗时 3-5 个工作日 | **适配销售快速出单**，提升商圈展业效率 |
| **理赔服务** | 小额理赔 (≤5万) 2-3 个工作日到账，线上拍照定损 | 常规理赔，需线下提交纸质材料，小额理赔 72 小时先行赔付 | **理赔流程更便捷**，客户体验更佳，易形成口碑传播 |
| **增值服务** | 附赠餐饮安全线上培训、后厨燃气检测等专属服务 | 无任何增值服务，仅基础保险赔付 | **服务绑定提升产品附加值**，增强客户粘性 |
| **定价灵活性** | 连锁品牌 3 家以上分店保费直降 20%，支持总部统保团购定价 | 单店统一定价，无团购/连锁品牌优惠 | **适配连锁餐饮批量投保**，降低客户整体投保成本 |
...
**是否选定该产品并匹配相应的营销素材？**`,
    ...
    // 6. Strategy: Select Product & Match Materials
    else if (stage === 7 && (userInput.includes('选定') || userInput.includes('是') || userInput.includes('好') || userInput.includes('要') || userInput.includes('匹配'))) {
      const msg1: Message = {
        id: id(),
        role: 'assistant',
        content: "已为您选定产品并匹配相关营销素材（包含：老字号焕新海报、连锁统保案例H5、消防安全培训课件等）。\n\n接下来是否生成该商圈的**专属微店**？微店将包含上述推荐产品套餐、增值服务及营销素材。",
      };
      setMessages(prev => [...prev, msg1]);
      setStage(8);
    }
    // 7. Strategy: Generate Micro-store
    else if (stage === 8 && (userInput.includes('生成') || userInput.includes('是') || userInput.includes('好') || userInput.includes('要'))) {
      const msg1: Message = {
        id: id(),
        role: 'assistant',
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 font-sans text-gray-900 max-w-[375px] mx-auto overflow-hidden relative border-x border-gray-200 shadow-2xl">
      ...
      <div 
        ref={scrollRef}
        className="flex-1 overflow-y-auto p-2 space-y-4 no-scrollbar bg-[#f2f2f2] overflow-x-hidden"
      >
              ...
              >
                <div className={`flex gap-3 max-w-[95%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                  ...
                  <div className="flex flex-col gap-1 min-w-0">
                    <div className={`relative px-3 py-2 rounded-lg text-sm leading-normal shadow-sm max-w-full ${
                      msg.role === 'user' 
                          ...
                          >
                            <div className="flex items-center gap-2">
                              <div className="w-6 h-6 rounded-md bg-white flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform">
                                {i === 0 ? <BarChart3 size={14} className="text-blue-500" /> : 
                                 i === 1 ? <Store size={14} className="text-green-500" /> : 
          ...
          >
            <div className="relative w-full max-w-2xl h-[90vh] bg-white rounded-2xl overflow-hidden flex flex-col">
              <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-white shrink-0">
                ...
                <div className="flex items-center gap-2">
                  <button className="px-3 py-1.5 hover:bg-blue-50 rounded-lg transition-colors text-blue-600 flex items-center gap-1.5 border border-blue-100">
                    <Package size={18} />
              ...
              <div className="flex-1 overflow-y-auto p-6 bg-white prose prose-sm max-w-none">
                {selectedImage === 'REPORT_MODAL' && (
                  <div className="markdown-body">
                    <Markdown remarkPlugins={[remarkGfm]}>{FULL_REPORT_CONTENT}</Markdown>
                  </div>
                )}
              </div>
